...
function filterFloatTypes(geoJsonFeature, floatTypeString) {
...
let sliderSelection = document.querySelector("#slider").value;
let yearToPlot = "";
let yearToPlotNum = 0;
console.log("test");
  ...
  const renderedLayer = L.geoJson(jsonData, {
    filter: function (geoJsonFeature) {
      ...
      if (floatTypeString === "Regular") {
        if (geoJsonFeature.properties.floatType === floatTypeString) {
          //console.log("In true");
          // returning true means that this feature is retained through the filter
          return true;
        }
      }
      ...
      if (floatTypeString === "Fancy") {
        if (
          geoJsonFeature.properties.floatType === floatTypeString ||
          geoJsonFeature.properties.floatType === "Rona" ||
          geoJsonFeature.properties.floatType === "Pumpkin" ||
          geoJsonFeature.properties.floatType === "Fancy_2021_17"
        ) {
          console.log(geoJsonFeature.properties.floatType);
          return true;
        }
    ...
    pointToLayer: function (geoJsonPoint, latlng) {
      ...
      const numFound = geoJsonPoint.properties.numFound;
      ...
      const regularIconSize_x = 240;
      // Average of the 4 icon heights
      const regularIconSize_y = 263;
      ...
      let floatIcon = L.icon({
        ...
        // Size [x,y] in pixels
        iconSize: [
          regularIconSize_x * scaleFactor,
          regularIconSize_y * scaleFactor,
        ],
        ...
        iconAnchor: [
          (regularIconSize_x * scaleFactor) / 2,
          regularIconSize_y * scaleFactor,
        ],
        //popupAnchor:
      });
    ...
    onEachFeature: function (feature, layer) {
      const locationName = feature.properties.locationName;
      const numFloatsFound = feature.properties.numFound;
      const floatType = feature.properties.floatType;
      ...
      layer.bindPopup(
        `Location: <b>${locationName}</b></br>Float type: <b>${floatType}</b></br>Number found: <b>${numFloatsFound}</b>`
      );
    },
  });
...
function plotPoints(url, plotReg, plotFancy) {
  fetch(url)
    ...
    .then((jsonData) => {
      // Also functional, but removing jQuery for now.
...
let regularFloatCheck = document.querySelector("#regular");
//console.log(regularFloatCheck);
let fancyFloatCheck = document.querySelector("#fancy");
...
function redrawFloats() {
  ...
  sliderSelection = document.querySelector("#slider").value;
